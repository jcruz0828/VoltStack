...
@Component({
  ...
  template: `
    <div class="flex flex-col min-h-screen">
      ...
      <main class="flex-1 pt-20 pb-20 bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-800">
        <div class="container mx-auto px-4 py-8">
          <div class="max-w-md mx-auto space-y-8 bg-white p-8 rounded-xl shadow-lg dark:bg-gray-900 dark:border dark:border-gray-800" [@fadeInUp]>
            <!-- Header -->
            ...
            <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="mt-8 space-y-6">
              ...
              <!-- Gmail Tracking Option -->
              <div *ngIf="isGmail" class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div class="flex items-start">
                  <div class="flex items-center h-5">
                    <input id="enableTracking" type="checkbox" formControlName="enableTracking"
                      class="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700">
                  </div>
                  <div class="ml-3">
                    <label for="enableTracking" class="text-sm font-medium text-blue-800 dark:text-blue-300">
                      Enable Automatic Gmail Tracking
                    </label>
                    <p class="text-sm text-blue-600 dark:text-blue-400">
                      Allow us to automatically track your job application status updates from your Gmail account.
                    </p>
                  </div>
                </div>
              </div>
...
export class RegisterComponent implements OnInit {
  ...
  constructor(
    ...
    // Subscribe to email changes to check if it's a Gmail address
    this.registerForm.get('email')?.valueChanges.subscribe(email => {
      this.isGmail = email?.toLowerCase().endsWith('@gmail.com');
      if (!this.isGmail) {
        this.registerForm.get('enableTracking')?.setValue(false);
      }
    });
  ...
  onSubmit() {
    if (this.registerForm.valid) {
      this.isLoading = true;
      const { firstName, lastName, email, password, enableTracking } = this.registerForm.value;

      // If user enabled Gmail tracking, redirect to Google OAuth
      if (enableTracking && this.isGmail) {
